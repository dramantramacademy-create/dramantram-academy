...
const COURSES = [
  {
    ...
    features: ['Graphic design fundamentals', 'Branding & visual identity', 'Portfolio support'],
    color: '#FF0000',
    icon: PenTool
  },
  {
    ...
    features: ['UI basics using Figma', 'Wireframing & prototyping', 'User research'],
    color: '#FF0000',
    icon: Figma
  },
  {
    ...
    features: ['Motion graphics', 'After Effects & Premiere', '3D fundamentals'],
    color: '#FF0000',
    icon: Video
  },
  {
    ...
    features: ['Editing Fundamentals', 'Storytelling logic', 'Commercial Workflows'],
    color: '#FF0000',
    icon: MonitorPlay
  }
];
...
const Courses: React.FC = () => {
  return (
    <section id="courses" className="py-32 relative overflow-hidden bg-black reveal">
      <div className="absolute inset-0 bg-red-glow opacity-30"></div>
      
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="text-center mb-20">
          <div className="text-[11px] font-black text-[#FF0000] uppercase tracking-[0.5em] mb-4">OUR PROGRAMS</div>
          <h2 className="oswald text-5xl md:text-8xl font-black uppercase mb-8 leading-[0.9] tracking-tighter">
            Programs <br /><span className="text-gradient-red">Overview.</span>
          </h2>
          <p className="text-gray-400 max-w-2xl mx-auto font-medium text-lg leading-relaxed">
            Intensive bootcamps designed to transform your passion into a professional edge. No theory overload, just pure skill acquisition.
          </p>
        </div>

        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {COURSES.map((course) => (
            <div key={course.id} className="group relative gloss-card rounded-[2rem] p-8 hover:translate-y-[-10px] flex flex-col items-start text-left border-red-900/10 overflow-hidden">
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <div className="text-[9px] font-black uppercase tracking-[0.3em] text-[#FFD700]">{course.category}</div>
              </div>
